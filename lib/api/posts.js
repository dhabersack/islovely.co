...
import { getPostCategoryByTitle } from '@/lib/api/post-categories'
...
export async function getAllPosts() {
  ...
  return (await Promise.all(directories.map(async ({
    ...
    const categories = await Promise.all(data.categories?.map(async categoryTitle => await getPostCategoryByTitle(categoryTitle)))
